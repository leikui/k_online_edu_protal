  ...
  export default {
    ...
    methods: {
      ...
      toPay() {
        ...
        // console.log("参数数：：" + this.$route.query.order.orderNo)
        // this.orderInfo = this.$route.query.order
        orderApi.orderPay(this.order)
          .then(resp => {
            this.payForm = resp.data.payForm
            console.log(resp.data.payForm)
            // this.orderNo = resp.data.orderNo
            // this.$router.push({path: '/pay',query: {'payForm':this.payForm}})
            this.$router.push({path:'/pay',query:{ 'payForm':this.payForm}})
